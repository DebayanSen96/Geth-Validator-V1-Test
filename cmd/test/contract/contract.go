...
func init() {
	...
	rootCmd.AddCommand(checkDXPCmd)
...
func registerVerifier() {
	...
	if err != nil {
		...
		fmt.Println("Run './bin/contract-test check-dxp' for more details.")
		return
	}
...
func checkDXPTokens() {
	...
	if balance.Cmp(minStake) < 0 {
		...
		fmt.Println("Run the 'approve' command to set the proper allowance.")
	} else {
...
func approveDXPTokens() {
	...
	tokenContract := bind.NewBoundContract(tokenAddress, parsedABI, client, client, client)
	if err != nil {
		log.Fatalf("Failed to create token contract instance: %v", err)
	}
	...
	amountInWei := new(big.Int).Mul(big.NewInt(approvalAmount), big.NewInt(1000000000000000000)) // 10^18

	fmt.Printf("Approving %d DXP tokens (%s wei) for contract %s...\n", approvalAmount, amountInWei.String(), contractAddr)

	// Call approve function
	contractAddress := common.HexToAddress(contractAddr)
	tx, err := tokenContract.Transact(auth, "approve", contractAddress, amountInWei)
	if err != nil {
	...
	fmt.Println("Check the transaction status on Sepolia block explorer")
	fmt.Println("After approval is confirmed, you can register as a verifier")
}
